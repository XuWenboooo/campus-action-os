...
function actionView(action) {
  ...
  return {
    ...
    condition: (action.conditions || []).map((item) => item.statement || item).join('；') || action.condition || '无额外条件',
    materials: (action.required_materials || []).map((item) => item.description || item).join('、') || action.materials || '无',
    steps: (action.steps || []).map((item) => item.instruction || item),
    statusLabel: action.verification_status === 'user_confirmation_required' ? '有信息需要你确认' : '已根据通知编译',
    statusTone: action.verification_status === 'user_confirmation_required' ? 'orange' : 'green',
  };
}

Page({
  data: { action: null, actionView: null, assessment: null, sourceText: '', loading: true, creating: false, error: '', orchestration: false, orchestrationActions: [], relations: [], prioritySuggestion: null, conflicts: [], changeImpacts: [] },
  onLoad(options) {
    ...
    loader.then((payload) => {
      ...
      this.setData({ action, actionView: actionView(action), assessment: result.document_assessment, sourceText: result.source_text || (action && action.evidence && action.evidence[0] && action.evidence[0].source_text) || '', loading: false, orchestration: Boolean(result.orchestration), orchestrationActions: result.orchestration_actions || result.verified_actions || [], relations: result.relations || [], prioritySuggestion: result.priority_suggestion || null, conflicts: result.conflicts || [], changeImpacts: result.change_impacts || [] });
