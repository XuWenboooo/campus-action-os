...
Page({
  ...
  onLoad(options) { this.changeEventId = options && options.changeEventId; this.taskId = options && options.taskId; api.setDiffContext(this.taskId); this.load(); },
  ...
  apply() { this.setData({ applying: true, error: '' }); api.applyNotificationDiff(this.changeEventId).then((result) => { this.setData({ applied: true }); wx.showToast({ title: '任务已同步更新', icon: 'success' }); const taskId = result.task_id || (result.task && result.task.task_id) || this.taskId; setTimeout(() => wx.navigateTo({ url: `/pages/task-detail/task-detail?taskId=${taskId}` }), 550); }).catch(() => this.setData({ error: '同步失败，请重试。' })).finally(() => this.setData({ applying: false })); },
