...
module.exports = {
  ...
  createDocument(text) { const state = readState(); const id = `doc-${Date.now()}`; const scenario = pickScenario(text); state.documents[id] = { document_id: id, text, scenario }; state.scenario = scenario; writeState(state); return Promise.resolve({ document: clone(state.documents[id]) }); },
