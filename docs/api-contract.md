...
Every response includes `x-request-id`. Errors use `{ "error": { "code", "message", "requestId", "retryable" } }`. State-changing POST operations require `Idempotency-Key`; a replay returns the original response, while reuse with a different request is a `409 IDEMPOTENCY_CONFLICT`. Development login is available only outside `APP_ENV=production`.
