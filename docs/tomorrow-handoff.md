...
当前工作树包含用户生成且未跟踪的 `output/`、`tmp/` 和 `tools/create_*_pdf.py`，不要删除或提交。工程代码已完成本地 SQLite/AI/API 文本闭环，并补充了评测指标、文件持久化验证、HTML 标准化、图片/PDF 的明确 OCR 降级、条件 Action Graph 分支和通知修订到既有 Task 的待确认同步，下一轮优先：
